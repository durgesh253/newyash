...
//     if (voice_id) {
//       await client.agent.update(override_agent_id || "agent_1d0292b304cceaccd4a0044755", {
...
//     const updateUrl = `https://api.retellai.com/update-conversation-flow/conversation_flow_436a875b65e6`;
...
//       to_number: to_number,
//       override_agent_id: override_agent_id || "agent_1d0292b304cceaccd4a0044755",
//     });
...
/**
 * Fetches all reservations from the Hospitable API
 */
async function getAllReservations(options = {}) {
  try {
    const bearerToken = process.env.HOSPITABLE_API_KEY;
    ...
    while (true) {
      ...
      if (queryParams.date_query) queryString.append("date_query", queryParams.date_query);

      const url = `https://public.api.hospitable.com/v2/reservations?${queryString.toString()}`;
...
exports.getAllReservations = async (req, res) => {
  try {
    ...
    const options = {
      ...
      date_query: req.query.date_query || "checkin",
    };

    const reservations = await getAllReservations(options);
    res.status(200).json({
      success: true,
      data: reservations,
    });
...
exports.makeReservationCall = async (req, res) => {
  try {
    ...
    global_prompt = global_prompt.replace(/%arrival_date%/g, formattedArrival);
    global_prompt = global_prompt.replace(/%departure_date%/g, formattedDeparture);
    global_prompt = global_prompt.replace(/%check_in%/g, formattedCheckIn);
    global_prompt = global_prompt.replace(/%check_out%/g, formattedCheckOut);
    global_prompt = global_prompt.replace(/%guest_count%/g, reservationData.guests?.total || 0);
...
Do NOT fabricate missing data. If a value is not present, return null.`,
    ...
    const updateData = {
      ...
      id: callDetail?.retell_key,
      call_id: callDetail?.call_id,
      inquiry_id: callDetail?.inquiry_id || null,
    };
